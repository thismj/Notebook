...
指针碰撞：堆的内存是绝对规整的，通过一个指针作为分界点区分已用内存和空闲内存，则一个对象的内存分配就是把指针朝空闲内存区域挪动一段与该对象大小相等的距离。

空闲列表：堆的内存不是规整的，虚拟机维护了一个空闲列表，记录哪些内存块可用，给一个对象分配内存的时候从列表中找到一块足够大的空间划分给该对象使用。

## GC

### 判断对象是否存活

* 引用计数算法：给每个对象添加引用计数器，对象被引用时计数器+1，引用失效时计数器-1，则引用为0的对象不可能再被使用。解决不了对象间循环引用问题。
* 可达性分析算法：通过 `GC Roots`作为起点，基于对象引用关系，向下遍历，所走过的路径称之为引用链，当一个对象到 `GC Roots`没有任何引用链相连，则证明该对象是不可能再被使用的。

### GC Roots包括哪些

* 所有`Java`线程当前活跃的栈帧里指向`GC`堆里的对象的引用；换句话说，当前所有正在被调用的方法的引用类型的参数/局部变量/临时值
* `VM`的一些静态数据结构里指向`GC`堆里的对象的引用，例如说`HotSpot VM`里的`Universe`里有很多这样的引用
* `JNI handles`，包括`global handles`和`local handles`
*  `synchronized`锁住的对象引用
* （看情况）所有当前被加载的`Java`类
* （看情况）`Java`类的引用类型静态变量
...
线程私有的内存区域，例如程序计数器、`Java`虚拟机栈、`Native`方法栈，同线程生命周期共生死，无需 `GC`；线程共享的方法区、堆则是 `GC`关注的重点。

### GC算法

* **mark-sweep** 标记清除算法

  标记需要清理的对象，然后直接回收，优点是简单快速，缺点是会出现很多内存碎片

* **mark-copy** 标记复制算法

  内存对半分，总是保留一块空闲（假设为左侧），首先标记右侧那块需要清理的对象，然后把存活的对象复制到左侧，最后清理掉右侧空间。避免了内存碎片，但是内存浪费了50%

* **mark-compact** 标记整理算法

  先使用标记清除算法回收，然后把剩余的存活对象整理挪动，保证它们占用的空间连续，但是整理比较耗时会降低 `GC` 的效率

* **generation-collect** 分代收集算法

  经过大量的实际分析，发现内存中的对象大致可以分为两类：有些生命周期很短，例如局部变量/临时对象，有些则会存活很久，因此产生了分代的思想，以 Hotspot 为例：

  <img src="https://static001.infoq.cn/resource/image/e4/77/e4ff361d409b6939e6da49a06b6dc677.png" style="zoom:80%;" />

  分为：

  1. 青年代：eden+S0（from survivor）+S1（to survivor）
  2. 老年代：Tenured
  3. 永久代：Permanent（JDK 1.8 开始，MetaSpace 取代永久代）

  Hotspot 分代收集的主要过程：

  1. 开始时，对象分配在 eden 区，当 eden 放不下的时候就会发生 minor GC（young GC），通过标记复制算法，把存活对复制到 S0 区，此时 eden 空闲。
  2. 对象继续分配在 eden 区，如果 eden 区又满了，再次触发 minor GC，通过标记复制算法把 eden 和 S0 区的存活对象复制到 S1 区，此时 eden 和 S0 区空闲
  3. 随着对象的不断分配，eden 区又满了，此时重复之前的 minor GC，通过标记复制算法把 eden 和 S1 区的存活对象复制到 S0，此时 eden 和 S1 空闲
  4. ......（省略重复过程）
  5. 对象在青年代的各个区每移动一次则年龄 +1，当年龄超过一定的阈值之后，在下次 minor GC 发生时会晋升并移动到老年代（如果 eden 区放不下大对象，会省略晋升过程直接分配在老年代）
  6. 随着时间的推移，如果老年代也满了，则会发生 major GC（full GC，标记-清理-整理算法），比较耗时
